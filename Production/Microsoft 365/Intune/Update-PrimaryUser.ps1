...
[string]$deviceName
    ...
    try {
        ...
        if($deviceName){
            ...
            #$Resource = "deviceManagement/managedDevices?`$filter=(((deviceType%20eq%20%27desktop%27)%20or%20(deviceType%20eq%20%27windowsRT%27)%20or%20(deviceType%20eq%20%27winEmbedded%27)%20or%20(deviceType%20eq%20%27surfaceHub%27)%20or%20(deviceType%20eq%20%27windows10x%27)))"
            $Resource = "deviceManagement/managedDevices?`$filter=(((chassisType%20eq%20%27desktop%27)%20or%20(chassisType%20eq%20%27laptop%27)%20or%20(chassisType%20eq%20%27worksWorkstation%27)))"
	        $uri = "https://graph.microsoft.com/$graphApiVersion/$($Resource)"
...
Foreach ($Device in $Devices){
        ...
        $Device10 = Get-Win10IntuneManagedDevice -deviceName $device.deviceName

        Write-Host "Device name:" $device10."deviceName" -ForegroundColor Cyan -ErrorAction Ignore
        $IntuneDevicePrimaryUser = Get-IntuneDevicePrimaryUser -deviceId $Device10.id -ErrorAction Ignore

        #Check if there is a Primary user set on the device already
        if($IntuneDevicePrimaryUser -eq $null){

            Write-Host "No Intune Primary User Id set for Intune Managed Device" $Device10."deviceName" -f Red -ErrorAction Ignore

        }

        else {
            $PrimaryAADUser = Get-AADUser -userPrincipalName $IntuneDevicePrimaryUser -ErrorAction Ignore
            Write-Host "Intune Device Primary User:" $PrimaryAADUser.displayName -ErrorAction Ignore

        }

        #Get the objectID of the last logged in user for the device, which is the last object in the list of usersLoggedOn
        $LastLoggedInUser = ($Device10.usersLoggedOn[-1]).userId

        #Using the objectID, get the user from the Microsoft Graph for logging purposes
        $User = Get-AADUser -userPrincipalName $LastLoggedInUser -ErrorAction Ignore
            ...
            if($IntuneDevicePrimaryUser -notmatch $User.id){
                ...
                $SetIntuneDevicePrimaryUser = Set-IntuneDevicePrimaryUser -IntuneDeviceId $Device10.id -userId $User.id -ErrorAction Ignore

                if($SetIntuneDevicePrimaryUser -eq ""){

                    Write-Host "User"$User.displayName"set as Primary User for device '$($Device10.deviceName)'..." -ForegroundColor Green -ErrorAction Ignore
